...
// 1ページあたりの表示件数
const MAX_LIST_NOLOGIN = 24;
// ページネーションの上限
const MAX_PAGE_OFFSET_NOLOGIN = 10000;

// ブラウザからのアクセスがあったとき
function n3s_web_list_nologin()
{
    $r = n3s_list_nologin_get();
    n3s_template_fw('list_nologin.html', $r);
}

// データを取得する
function n3s_list_nologin_get()
{
    global $n3s_config;
    ...
    if ($offset > MAX_PAGE_OFFSET_NOLOGIN) {
        $offset = MAX_PAGE_OFFSET_NOLOGIN;
    }
    ...
    $list = db_get($sql, array_merge($where_params, [MAX_LIST_NOLOGIN, $offset]));
    ...
    $prev_url = null;
    if ($offset > 0) {
        $prev_offset = max(0, $offset - MAX_LIST_NOLOGIN);
        $prev_url = n3s_getURL('all', 'list_nologin', [
            'sort' => $sort,
            'offset' => $prev_offset,
            'nofilter' => $nofilter,
            'onlybad' => $onlybad,
    ...
    $next_url = null;
    if ($offset + MAX_LIST_NOLOGIN < $total_count) {
        $next_offset = $offset + MAX_LIST_NOLOGIN;
        $next_url = n3s_getURL('all', 'list_nologin', [
            'sort' => $sort,
            'offset' => $next_offset,
            'nofilter' => $nofilter,
            'onlybad' => $onlybad,
