...
test('非公開・限定公開作品に対するコメント閲覧制限', function () {
    // 1. 非公開作品 (is_private = 1, user_id = 2) を登録
    $app_id_private = db_insert(
        "INSERT INTO apps (user_id, title, author, is_private, editkey, ctime, mtime) VALUES (2, '非公開作品', '次郎', 1, 'privatekey', 123, 123)",
        [],
        'main'
    );
    
    // 2. 限定公開作品 (is_private = 2, user_id = 2) を登録
    $app_id_limited = db_insert(
        "INSERT INTO apps (user_id, title, author, is_private, editkey, ctime, mtime) VALUES (2, '限定公開作品', '次郎', 2, 'limitedkey', 123, 123)",
        [],
        'main'
    );
    
    // それぞれの作品にコメントを登録しておく
    db_insert("INSERT INTO comments (user_id, app_id, parent_id, name, body, status, fav, ctime) VALUES (1, ?, 0, '太郎', '非公開へのコメント', 'approved', 0, 123)", [$app_id_private], 'main');
    db_insert("INSERT INTO comments (user_id, app_id, parent_id, name, body, status, fav, ctime) VALUES (1, ?, 0, '太郎', '限定公開へのコメント', 'approved', 0, 123)", [$app_id_limited], 'main');

    // -- 非公開作品のコメント取得テスト --
    
    // A. ログインなし、キーなし → 閲覧制限で拒否されること
    $_GET = ['action' => 'comment', 'mode' => 'list', 'app_id' => $app_id_private];
    $_POST = [];
    $_REQUEST = $_GET;
    $out = n3s_test_capture(fn() => n3s_api_comment());
    $res = json_decode($out, true);
    expect($res['result'])->toBe(false);
    expect($res['msg'])->toBe('この作品の閲覧権限がありません。');
    
    // B. 正しい editkey を提示しても、非公開(1)の場合は第三者なので拒否されること
    $_GET = ['action' => 'comment', 'mode' => 'list', 'app_id' => $app_id_private, 'editkey' => 'privatekey'];
    $_REQUEST = $_GET;
    $out = n3s_test_capture(fn() => n3s_api_comment());
    $res = json_decode($out, true);
    expect($res['result'])->toBe(false);
    ...
    // C. 投稿者本人 (user_id = 2) としてログインした場合は閲覧できること
    $_SESSION['n3s_login'] = true;
    $_SESSION['user_id'] = 2;
    $_GET = ['action' => 'comment', 'mode' => 'list', 'app_id' => $app_id_private];
    $_REQUEST = $_GET;
    ...
    expect($res['result'])->toBe(true);
    expect(count($res['comments']))->toBe(1);
    expect($res['comments'][0]['body'])->toBe('非公開へのコメント');
    $_SESSION = []; // テスト用ログアウト
    
    // -- 限定公開作品のコメント取得テスト --
    
    // A. ログインなし、キーなし → 閲覧制限で拒否されること
    $_GET = ['action' => 'comment', 'mode' => 'list', 'app_id' => $app_id_limited];
    $_REQUEST = $_GET;
    $out = n3s_test_capture(fn() => n3s_api_comment());
    $res = json_decode($out, true);
    expect($res['result'])->toBe(false);
    
    // B. 正しい editkey を提示した場合は閲覧できること
    $_GET = ['action' => 'comment', 'mode' => 'list', 'app_id' => $app_id_limited, 'editkey' => 'limitedkey'];
    $_REQUEST = $_GET;
    $out = n3s_test_capture(fn() => n3s_api_comment());
    $res = json_decode($out, true);
    expect($res['result'])->toBe(true);
