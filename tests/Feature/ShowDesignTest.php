...
test('作品表示ページは共通デザインと既存の操作契約を維持する', function () {
    ...
    n3s_set_config('comment_templates', [42 => '設定から生成したコメント']);
    $userId = n3s_add_user('show-design@example.com', 'password1', '表示太郎');
    expect(n3s_login('show-design@example.com', 'password1'))->toBeTrue();
    ...
    $imageId = db_insert(
        ...
        ['作品の扉絵', '501.jpg', $userId, 'SELF', $now, $now]
    );

    $appId = db_insert(
        'INSERT INTO apps (title, author, memo, user_id, is_private, nakotype, copyright, tag, version, image_id, ctime, mtime) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
        ['表示デザインテスト', '表示太郎', '作品表示ページの説明文です。', $userId, 0, 'wnako', 'MIT', 'テスト,表示', '3.7.2', $imageId, $now, $now]
    );
    ...
    expect($out)
        ...
        ->toContain('<meta property="og:image" content="http://localhost:7450/repos/nako3storage/image.php?f=501.jpg">')
