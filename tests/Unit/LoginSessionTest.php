<?php

test('正しいメールアドレスとパスワードでログインするとセッションにログイン情報が保存される', function () {
    // n3s_add_user() は PDO::lastInsertId() 由来の文字列を返すため、int化して比較する
    $user_id = (int) n3s_add_user('login@example.com', 'correct-horse', 'ログインユーザー');

    $ok = n3s_login('login@example.com', 'correct-horse');

    expect($ok)->toBeTrue()
        ->and($_SESSION['n3s_login'])->toBeTrue()
        ->and($_SESSION['user_id'])->toBe($user_id)
        ->and($_SESSION['name'])->toBe('ログインユーザー')
        ->and($_SESSION['screen_name'])->toBe('')
        ->and($_SESSION['profile_url'])->toBe('https://n3s.nadesi.com/image.php?f=726.png');
});

test('パスワードが間違っているとログインは失敗し、セッションは変更されない', function () {
    n3s_add_user('login2@example.com', 'correct-horse', 'ユーザー');

    $ok = n3s_login('login2@example.com', 'wrong-password');

    expect($ok)->toBeFalse()
        ->and($_SESSION)->not->toHaveKey('n3s_login');
});

test('存在しないメールアドレスはログインに失敗する', function () {
    $ok = n3s_login('nobody@example.com', 'whatever');

    expect($ok)->toBeFalse();
});

test('salt列が空の既存ユーザー (def:: 後方互換) でもログインできる', function () {
    n3s_test_add_legacy_user('legacy@example.com', 'legacy-pass', 'レガシー');

    $ok = n3s_login('legacy@example.com', 'legacy-pass');

    expect($ok)->toBeTrue()
        ->and($_SESSION['n3s_login'])->toBeTrue();
});

test('def:: 後方互換ユーザーも誤ったパスワードでは失敗する', function () {
    n3s_test_add_legacy_user('legacy2@example.com', 'legacy-pass', 'レガシー');

    $ok = n3s_login('legacy2@example.com', 'incorrect');

    expect($ok)->toBeFalse();
});

test('旧方式 (salt:: 形式) のユーザーはログイン成功時に自動でpassword_hash()形式へ移行する', function () {
    // 平文パスワードはログイン成功時にしか手に入らないため、旧方式のハッシュを
    // 使い続けるしかなかった既存ユーザーも、次回ログイン時に自動で新方式へ
    // 移行できるようにしてある (段階的移行。全ユーザーへの強制パスワード再設定は不要)。
    $user_id = n3s_add_user('upgrade1@example.com', 'upgrade-pass', 'アップグレード太郎');
    // 新規作成直後は既に hash:: 形式なので、いったん旧方式のハッシュに戻して検証する
    $salt = n3s_generate_salt();
    $legacyHash = n3s_login_password_to_hash('upgrade-pass', $salt);
    db_exec('UPDATE users SET password=?, salt=? WHERE user_id=?', [$legacyHash, $salt, $user_id], 'users');

    $before = db_get1('SELECT password FROM users WHERE user_id=?', [$user_id], 'users');
    expect($before['password'])->toStartWith('salt::');

    $ok = n3s_login('upgrade1@example.com', 'upgrade-pass');

    expect($ok)->toBeTrue();

    $after = db_get1('SELECT password, salt FROM users WHERE user_id=?', [$user_id], 'users');
    expect($after['password'])->toStartWith('hash::')
        ->and($after['salt'])->toBe('');

    // 移行後も同じパスワードで問題なくログインできる
    n3s_logout();
    expect(n3s_login('upgrade1@example.com', 'upgrade-pass'))->toBeTrue();
});

test('def:: 形式のユーザーもログイン成功時にpassword_hash()形式へ移行する', function () {
    n3s_test_add_legacy_user('upgrade2@example.com', 'legacy-pass', 'アップグレード二郎');

    $ok = n3s_login('upgrade2@example.com', 'legacy-pass');

    expect($ok)->toBeTrue();

    $user_id = n3s_get_user_id_by_email('upgrade2@example.com');
    $after = db_get1('SELECT password FROM users WHERE user_id=?', [$user_id], 'users');
    expect($after['password'])->toStartWith('hash::');
});

test('長年運用してきたDBを想定: 旧方式ユーザーの移行は password/salt 以外のカラムを一切変更しない', function () {
    // 実際の app/sql/init-users.sql (n3s_test_setup() が使うのと同じスキーマ、
    // 本番と同じくマイグレーションSQL等は無い) に対して、何年も前に登録された
    // ユーザーを模した行を直接INSERTし、ログインによる自動移行が
    // 他のプロフィール情報を一切壊さないことを確認する。
    $salt = n3s_generate_salt();
    $hash = n3s_login_password_to_hash('oldPassw0rd!', $salt);
    $user_id = db_insert(
        'INSERT INTO users ' .
        '(email, password, name, screen_name, description, twitter_id, profile_url, salt, ctime, mtime) ' .
        'VALUES (?,?,?,?,?,?,?,?,?,?)',
        [
            'veteran@example.com', $hash, '古参ユーザー', 'veteran_sn',
            '昔から使っています', 12345, 'https://example.com/icon.png', $salt,
            1000000000, 1000000000,
        ],
        'users'
    );

    $before = db_get1('SELECT * FROM users WHERE user_id=?', [$user_id], 'users');
    expect($before['password'])->toStartWith('salt::');

    expect(n3s_login('veteran@example.com', 'oldPassw0rd!'))->toBeTrue();

    $after = db_get1('SELECT * FROM users WHERE user_id=?', [$user_id], 'users');
    expect($after['password'])->toStartWith('hash::')
        ->and($after['salt'])->toBe('')
        // password/salt 以外は不変であること (プロフィール情報などを壊さない)
        ->and($after['email'])->toBe($before['email'])
        ->and($after['name'])->toBe($before['name'])
        ->and($after['screen_name'])->toBe($before['screen_name'])
        ->and($after['description'])->toBe($before['description'])
        ->and($after['twitter_id'])->toBe($before['twitter_id'])
        ->and($after['profile_url'])->toBe($before['profile_url'])
        ->and($after['ctime'])->toBe($before['ctime']);

    // 移行後も同じパスワードでログインでき続ける
    n3s_logout();
    expect(n3s_login('veteran@example.com', 'oldPassw0rd!'))->toBeTrue();
});

test('パスワード未設定 (password="") のユーザーはどんな入力でもログインできない', function () {
    // docs/user_login.md #4.5 手順5, #9: 新規登録直後はダミーパスワードのハッシュが
    // 設定され、パスワード設定フローを経るまで本来のログインはできない設計になっている。
    // ここではその前提となる「password列が空ならn3s_login()は常にfalseを返す」という
    // 不変条件をユニットレベルで検証する
    // (n3s_web_login_trylogin() 側のガード分岐は exit() するため直接は呼べない)。
    $user_id = db_insert(
        'INSERT INTO users (email, password, name, salt) VALUES (?,?,?,?)',
        ['nopassword@example.com', '', '未設定太郎', ''],
        'users'
    );
    expect($user_id)->toBeGreaterThan(0);

    expect(n3s_login('nopassword@example.com', ''))->toBeFalse()
        ->and(n3s_login('nopassword@example.com', 'anything'))->toBeFalse();
});

test('n3s_is_login / n3s_get_user_id はログイン前はfalse/0を返す', function () {
    expect(n3s_is_login())->toBeFalse()
        ->and(n3s_get_user_id())->toBe(0);
});

test('n3s_get_login_info はログイン前はデフォルト値を返す', function () {
    $info = n3s_get_login_info();

    expect($info)->toBe([
        'user_id' => 0,
        'name' => '?',
        'screen_name' => '?',
        'profile_url' => 'https://n3s.nadesi.com/image.php?f=726.png',
    ]);
});

test('n3s_get_login_info はログイン後はセッションの値を返す', function () {
    $user_id = (int) n3s_add_user('info@example.com', 'password1', '情報太郎');
    db_exec('UPDATE users SET screen_name=? WHERE user_id=?', ['info_taro', $user_id], 'users');
    n3s_login('info@example.com', 'password1');

    $info = n3s_get_login_info();

    expect($info)->toBe([
        'user_id' => $user_id,
        'name' => '情報太郎',
        'screen_name' => 'info_taro',
        'profile_url' => 'https://n3s.nadesi.com/image.php?f=726.png',
    ]);
});

test('n3s_is_admin は admin_users に含まれるユーザーIDのみtrueを返す', function () {
    n3s_test_setup(['admin_users' => [42]]);
    $admin_id = n3s_add_user('admin@example.com', 'adminpass', '管理者');
    $user_id = n3s_add_user('normal@example.com', 'userpass', '一般ユーザー');

    // 管理者IDを42に固定できないため、42になりきったセッションを模擬する
    $_SESSION['n3s_login'] = true;
    $_SESSION['user_id'] = 42;
    expect(n3s_is_admin())->toBeTrue();

    $_SESSION['user_id'] = $user_id;
    expect(n3s_is_admin())->toBeFalse();

    unset($admin_id);
});

test('n3s_logout はログイン関連のセッションを消去する', function () {
    n3s_add_user('logout@example.com', 'password1', 'ログアウト太郎');
    n3s_login('logout@example.com', 'password1');
    n3s_setBackURL('index.php?action=list');

    n3s_logout();

    expect(n3s_is_login())->toBeFalse()
        ->and($_SESSION)->not->toHaveKeys(['n3s_login', 'user_id', 'n3s_backurl', 'name']);

    $log = db_get1("SELECT * FROM logs WHERE kind='logout' ORDER BY log_id DESC LIMIT 1", [], 'log');
    expect($log)->not->toBeFalse();
});

test('ログインしていない状態で n3s_logout を呼んでもログは記録されない', function () {
    n3s_logout();

    $count = db_get1("SELECT count(*) AS c FROM logs WHERE kind='logout'", [], 'log');
    expect((int) $count['c'])->toBe(0);
});

test('n3s_get_user_name はログイン前は "?" を返す', function () {
    expect(n3s_get_user_name())->toBe('?');
});

test('n3s_get_user_name はログイン後、名前を正しく返す', function () {
    n3s_add_user('username@example.com', 'password1', '名前太郎');
    n3s_login('username@example.com', 'password1');

    expect(n3s_get_user_name())->toBe('名前太郎');
});
