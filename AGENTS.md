...
このリポジトリは、[なでしこv3](https://nadesi.com) のプログラムと関連リソースを保存・共有する Webサービスです。PHP7以上、SQLite、軽量フレームワーク `fw_simple` を前提にしています。

## AIエージェント

AIエージェントがこのリポジトリで作業する時は、まずこのファイルを読み、プロジェクト構成、アクションの流れ、テンプレート、DB、セッション・権限、検証方法を把握してください。

なお、AIエージェントは、日本語で応答してください。また、実装する前に計画を立てて、必要に応じて質問してください。

タスクランナーの`just`を使って、各種処理を実行します。テストは`just test`で実行できます。コードを編集したら実行して結果を確認してください。
